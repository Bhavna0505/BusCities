...
public class drivertickets extends AppCompatActivity {
    ...
    RecyclerView RV;
    FirestoreRecyclerAdapter<history, drivertickets.historyViewHolder> adp;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_drivertickets);
        getSupportActionBar().setTitle("Tickets Booked");
        db = FirebaseFirestore.getInstance();
        RV = findViewById(R.id.driverbookedticket);
        SimpleDateFormat date = new SimpleDateFormat("yyyy-MM-dd");
        String currentDateandTime = date.format(new Date());
        //Query
        Query qr = db.collection("BookedTickets").whereEqualTo("BUS_NUMBER", BusnoDriver.driverbusno).whereEqualTo("DATE", currentDateandTime);
        //Recycler Options
        FirestoreRecyclerOptions<history> opt = new FirestoreRecyclerOptions.Builder<history>()
                .setQuery(qr, history.class)
                .build();
        adp = new FirestoreRecyclerAdapter<history, drivertickets.historyViewHolder>(opt) {
            @NonNull
            @Override
            public drivertickets.historyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
                View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.historylist, parent, false);
                return new drivertickets.historyViewHolder(view);
            }

            @Override
            protected void onBindViewHolder(@NonNull drivertickets.historyViewHolder historyViewHolder, int i, @NonNull history history) {
